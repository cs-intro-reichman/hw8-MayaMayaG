...
public class Network {
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    public String recommendWhoToFollow(String name) {
        ...
        for (int i = 0; i < userCount; i++) 
        {
            ...
            if (other != user) {
                ...
                {
                    maxMutualFollowers = mutualFollowersCount;
                    recommendedUser = other;
                }
            }
        }
    ...
    public String mostPopularUser() {
        int[] popularity = new int[userCount]; 
        ...
        {
            User user1 = users[i];
            for (int j = 0; j < user1.getfFollows().length; j++) 
            {
                String nameFollowee =  user1.getfFollows()[j];
                if (nameFollowee != "") 
                {
                    for (int k = 0; k < userCount; k++) 
                    {
                        if (users[k].getName().equals(nameFollowee)) 
                        {
                            popularity[k]++;
                        }
                    }       
